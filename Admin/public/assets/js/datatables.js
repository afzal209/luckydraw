...
//   },
//   "sPaginationType": "bootstrap",
//   // Don't rely on numeric targets for the first column because
//   // ResponsiveDatatablesHelper may insert an expand icon column at index 0.
//   // We'll disable sorting on the index column via class instead (see below).
//   "aaSorting": [[1, "asc"]],
//   "oLanguage": {
//     "sLengthMenu": "_MENU_ ",
//     "sInfo": "Showing <b>_START_ to _END_</b> of _TOTAL_ entries"
...
//   fnRowCallback: function (nRow) {
//     responsiveHelper.createExpandIcon(nRow);
//     // also make sure the index cell isn't sortable by adding a class to the TH:
//     // <th class="no-sort">#</th>
//   },
//   fnDrawCallback: function (oSettings) {
//     responsiveHelper.respond();

//     // --- Dynamic row numbers (page-aware) ---
//     var start = oSettings._iDisplayStart || 0;
//     // Fill only the cells with our class, so extra responsive columns don't affect us
//     $(oSettings.nTBody).find('tr').each(function (i) {
//       var cell = $(this).find('td.row-index')[0];
//       if (cell) cell.innerHTML = start + i + 1;
//     });
//   },
//   // Optional: disable sorting on the numbered and actions columns by header class
//   "aoColumnDefs": [
//     { "bSortable": false, "aTargets": ["no-sort"] }
//   ]
// });

// Make sure your header for the index column has class="no-sort":
// <th class="no-sort">#</th>
...
// });
	...
	$('#example1 th').click(function(e) { 
			$('#example1 .animate-progress-bar').each(function () {
				$(this).removeClass('progress-bar');
				$(this).css('width','0%');
       			$(this).css('width', $(this).attr("data-percentage"));
				$(this).addClass('progress-bar');
	...
	$('#example1_wrapper .dataTables_filter input').addClass("input-medium "); // modify table search input
    $('#example1_wrapper .dataTables_length select').addClass("select2-wrapper span12"); // modify table per page dropdown

	
	
	$('#example1 input').click( function() {
        $(this).parent().parent().parent().toggleClass('row_selected');
    });
